...
export default function ComplaintPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // AI-powered routing
      const { department } = await routeComplaint({
      ...
      const isPriority = Math.random() > 0.8; // Simulate priority
      const newComplaintData: Complaint = {
        id: newCaseId,
        category: category || 'Other',
        date: new Date().toISOString().split('T')[0],
        status: 'Submitted',
        department: department, // Use AI-suggested department
      ...
      console.error("Failed to route complaint:", error);
      toast({
        title: "Submission Failed",
        description: "There was an error routing your complaint. Please try again.",
        variant: "destructive",
      });
    } finally {
